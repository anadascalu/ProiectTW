 <div class="container">
 	<div class="row">
 		<div class="col-lg-12 text-center">
 			<h2 class="section-heading">La dispozitia dumneavoastra!</h2>
 			<hr class="primary">
 		</div>
 	</div>
 </div>
 <div class="container">
 	<div class="row">
 		<div class="col-lg-3 col-md-6 text-center">
 			<div class="service-box">
 				<i class="fa fa-4x fa-diamond text-primary sr-icons"></i>
 				<h3>Pericole</h3>
 				<p class="text-muted">Va putem informa legat de pericolele posibilelor cutremure!</p>
 			</div>
 		</div>
 		<div class="col-lg-3 col-md-6 text-center">
 			<div class="service-box">
 				<i class="fa fa-4x fa-paper-plane text-primary sr-icons"></i>
 				<h3>Adapost</h3>
 				<p class="text-muted">Oferim adapost in cazul unui cutremur!</p>
 			</div>
 		</div>
 		<div class="col-lg-3 col-md-6 text-center">
 			<div class="service-box">
 				<i class="fa fa-4x fa-newspaper-o text-primary sr-icons"></i>
 				<h3>Informatii</h3>
 				<p class="text-muted">Va oferim informatii despre familiile dumneavoastra si posibilitatea de a-i contacta!</p>
 			</div>
 		</div>
 		<div class="col-lg-3 col-md-6 text-center">
 			<div class="service-box">
 				<i class="fa fa-4x fa-heart text-primary sr-icons"></i>
 				<h3>Oferim ajutor</h3>
 				<p class="text-muted">Oferim ajutor in cazul oricaror probleme cauzate de cutremure!</p>
 			</div>
 		</div>
 	</div>
 </div>
 
 <section class="no-padding" id="portfolio">
 	<div class="container-fluid">
 		<div class="row no-gutter popup-gallery">
 			<div class="col-lg-4 col-sm-6">
 				<a href="img/Kath_n.jpg" class="portfolio-box" >
 					<img src="img/Kath_c.jpg" class="img-responsive" alt="">
 					<div class="portfolio-box-caption">
 						<div class="portfolio-box-caption-content">
 							<div class="project-category text-faded">
 								Kathmandu
 							</div>
 							<div class="project-name">
 								Inainte & Dupa
 							</div>
 						</div>
 					</div>
 				</a>
 			</div>
 			<div class="col-lg-4 col-sm-6">
 				<a href="img/Nag_n.jpg" class="portfolio-box">
 					<img src="img/Nag_c.jpg" class="img-responsive" alt="">
 					<div class="portfolio-box-caption">
 						<div class="portfolio-box-caption-content">
 							<div class="project-category text-faded">
 								Nagarkot
 							</div>
 							<div class="project-name">
 								Inainte & Dupa
 							</div>
 						</div>
 					</div>
 				</a>
 			</div>
 			<div class="col-lg-4 col-sm-6">
 				<a href="img/Sag_n.jpg" class="portfolio-box">
 					<img src="img/Sag_c.jpg" class="img-responsive" alt="">
 					<div class="portfolio-box-caption">
 						<div class="portfolio-box-caption-content">
 							<div class="project-category text-faded">
 								Sagarmatha
 							</div>
 							<div class="project-name">
 								Inainte & Dupa
 							</div>
 						</div>
 					</div>
 				</a>
 			</div>
 			<div class="col-lg-4 col-sm-6">
 				<a href="img/It_n.jpg" class="portfolio-box">
 					<img src="img/It_c.jpg" class="img-responsive" alt="">
 					<div class="portfolio-box-caption">
 						<div class="portfolio-box-caption-content">
 							<div class="project-category text-faded">
 								Itahari
 							</div>
 							<div class="project-name">
 								Inainte & Dupa
 							</div>
 						</div>
 					</div>
 				</a>
 			</div>
 			<div class="col-lg-4 col-sm-6">
 				<a href="img/Pit_n.jpg" class="portfolio-box">
 					<img src="img/Pit_c.jpg" class="img-responsive" alt="">
 					<div class="portfolio-box-caption">
 						<div class="portfolio-box-caption-content">
 							<div class="project-category text-faded">
 								Pithoragarh
 							</div>
 							<div class="project-name">
 								Inainte & Dupa
 							</div>
 						</div>
 					</div>
 				</a>
 			</div>
 			<div class="col-lg-4 col-sm-6">
 				<a href="img/Kot_n.jpg" class="portfolio-box">
 					<img src="img/Kot_c.jpg" class="img-responsive" alt="">
 					<div class="portfolio-box-caption">
 						<div class="portfolio-box-caption-content">
 							<div class="project-category text-faded">
 								Kothari
 							</div>
 							<div class="project-name">
 								Inainte & Dupa
 							</div>
 						</div>
 					</div>
 				</a>
 			</div>
 		</div>
 	</div>
 </section>

 <aside class="bg-dark">
 	<div class="container text-center">
 		<div class="call-to-action">
 			<h2>PENTRU ORICE PROBLEME, CONTACTEAZA-NE!</h2>
 			<a class="btn btn-default btn-xl sr-button" href="http://localhost/TW/about" >Contact!</a>

 		</div>
 	</div>
 </aside>